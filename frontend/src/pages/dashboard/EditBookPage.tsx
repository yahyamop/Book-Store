...
const EditBookPage = () => {
  ...
  const [isBookPending, setBookPending] = useState(false);
  ...
  const form = useForm<z.infer<typeof BookSchema>>({
    ...
    defaultValues: {
      title: book?.title || "", // No validation currently
      description: book?.description || "", // No validation currently
      author: book?.author || "", // No validation currently
      price: book?.price && String(book.price) || "1", // Consider changing to number for calculations
      discount: book?.discount && String(book?.discount)   || "0", // Consider changing to number for calculations
      coverImage: "", // Might want to consider validation for image format/size
      categoryId: book?.categoryId || "", // Likely needs validation for existing category options
      publishedDate: new Date(Date.now()) as Date, // Consider using z.date() for date manipulation
      totalPages: book?.title || "1", // Already defined as number
      stock: book?.title || "1", // Already defined as number
      soldItemsNumber: book?.title || "0", // Already defined as number
    },
  });
  ...
  return (
    <div className="border shadow-sm rounded-lg p-4 md:p-6 ">
      <Form {...form}>
        ...
        >
          <h1 className="text-center text-3xl font-semibold ">Edit</h1>
